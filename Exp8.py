...
X_1['Name']=stock_data['Unnamed: 0']
x_1=X_1.drop(['Name'],axis=1)
...
def app():
    ...
    buy_stocks = []
    buy_stocks_names = set()
    for index, row in X_1.iterrows():
        Stock_data = row[features].to_frame().T
        prediction = rfc_new.predict(Stock_data)
        if prediction == 1:
            name = row['Name']
            if name not in buy_stocks_names:
                importance_score = rfc_new.feature_importances_.sum()  # Compute the feature importance score for the stock
                buy_stocks.append({"Name": name, "Importance": importance_score})
                buy_stocks_names.add(name)
    buy_stocks = sorted(buy_stocks, key=lambda x: x["Importance"], reverse=True)[
                 :10]  # Sort the stocks based on their importance scores
    for stock in buy_stocks:
        st.write(f"{stock['Name']} - Buy/Hold")

    st.subheader("Top 10 Stocks to Sell")
    sell_stocks = []
    sell_stocks_names = set()
    for index, row in X_1.iterrows():
        sTock_data = row[features].to_frame().T
        prediction = rfc_new.predict(sTock_data)
        if prediction != 1:
            name = row['Name']
            if name not in sell_stocks_names:
                importance_score = rfc_new.feature_importances_.sum()  # Compute the feature importance score for the stock
                sell_stocks.append({"Name": name, "Importance": importance_score})
                sell_stocks_names.add(name)
    sell_stocks = sorted(sell_stocks, key=lambda x: x["Importance"], reverse=True)[
                  :10]  # Sort the stocks based on their importance scores
