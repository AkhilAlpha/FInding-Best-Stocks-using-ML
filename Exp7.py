...
def app():
    ...
    # Add a new column to the stock_new DataFrame to calculate the prediction score
    stock_new['Prediction Score'] = rfc_new.predict_proba(stock_new[features])[:, 1]

    # Display top 10 stocks to buy and sell based on prediction score
    st.subheader("Stock Recommendations to Buy/Hold")
    buy_stocks = stock_new[stock_new['Prediction Score'] > 0.6].sort_values(by='Prediction Score',
                                                                            ascending=False).head(10)
    for index, row in buy_stocks.iterrows():
        st.write(f"{row['Name']} - Buy/Hold ({row['Prediction Score']:.2f})")

    st.subheader("Stock Recommendations to Sell")
    sell_stocks = stock_new[stock_new['Prediction Score'] < 0.6].sort_values(by='Prediction Score',
                                                                             ascending=True).head(10)
    for index, row in sell_stocks.iterrows():
        st.write(f"{row['Name']} - Sell ({row['Prediction Score']:.2f})")
